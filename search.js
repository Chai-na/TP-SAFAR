document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById('search-form').addEventListener('submit', function(event) {
        ...
        fetch(`http://nathanael-spriet.fr:1111/schedules?stationFrom=${stationFrom}&stationTo=${stationTo}`)
            .then(response => response.json())
            .then(schedules => {
                if (schedules.length === 0) {
                    errorMessage.textContent = "Aucun résultat trouvé.";
                } else {
                    const filteredSchedules = filterSchedules(schedules, date, timeFrom);
...
function filterSchedules(schedules, date, timeFrom) {
    const selectedDateTime = new Date(`${date}T${timeFrom}`);
    return schedules.filter(schedule => {
        const scheduleDateTime = new Date(`${schedule.date}T${schedule.departureTime}`);
        return scheduleDateTime >= selectedDateTime;
    });
}
